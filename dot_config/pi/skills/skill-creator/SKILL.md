...
Skills can be placed in these locations:
...
```bash
# Create the skill directory with optional resource folders
mkdir -p <skill-path>/scripts
mkdir -p <skill-path>/references
mkdir -p <skill-path>/assets

# Create the SKILL.md file
touch <skill-path>/SKILL.md
```

Only create the resource directories (`scripts/`, `references/`, `assets/`) that will actually be used.
...
When editing the skill, remember that it is being created for another instance of the agent to use. Include information that would be beneficial and non-obvious to the agent. Consider what procedural knowledge, domain-specific details, or reusable assets would help another agent instance execute these tasks more effectively.
...
Do not include any other required fields in YAML frontmatter. Optional fields include `license`, `compatibility`, and `metadata`.

##### Body

Write instructions for using the skill and its bundled resources. The description must be 1-1024 characters.

#### Validate the Skill

Run the validation script to check for common errors:

```bash
~/.config/opencode/skill/skill-creator/scripts/validate-skill.sh <path-to-skill>
```

Or if validating a project-local skill:

```bash
~/.config/opencode/skill/skill-creator/scripts/validate-skill.sh .opencode/skill/<skill-name>
```

The script checks:
- SKILL.md exists with valid frontmatter
- `name` and `description` are present
- Name follows hyphen-case convention (1-64 chars)
- Name matches directory name
- Description is 1-1024 chars with no angle brackets
